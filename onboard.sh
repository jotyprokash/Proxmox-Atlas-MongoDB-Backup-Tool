#!/usr/bin/env bash
set -e

# Colors for better UX
GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

echo -e "${BLUE}==============================================${NC}"
echo -e "${BLUE}   Proxmox Atlas Backup - Developer Onboarding ${NC}"
echo -e "${BLUE}==============================================${NC}"

# 1. Check for Sudo
if [ "$EUID" -ne 0 ]; then
  echo "Please run as root or with sudo."
  exit 1
fi

# 2. Install System Dependencies
echo -e "\n${GREEN}[1/3] Checking system dependencies...${NC}"
# We must install 'make' first so we can run the Makefile
apt update && apt install -y make
make deps

# 3. Interactive Configuration
echo -e "\n${GREEN}[2/3] Configuring Atlas Connection...${NC}"
read -p "Enter your MongoDB Atlas URI: " USER_URI
read -p "Enter your Discord Webhook URL (optional, press Enter to skip): " WEBHOOK

if [ -z "$USER_URI" ]; then
    echo "No URI provided. Using existing config if available."
else
    # Install with the provided URI
    ATLAS_URI="$USER_URI" make install
    
    # Inject Webhook if provided
    if [ -n "$WEBHOOK" ]; then
        sed -i "s|# WEBHOOK_URL=.*|WEBHOOK_URL=\"$WEBHOOK\"|" /etc/atlas-backup/backup.conf
        echo "Discord notifications enabled."
    fi
fi

# 4. Finalize
echo -e "\n${GREEN}[3/3] Finalizing Installation...${NC}"
echo -e "${BLUE}Success! The backup service is now active.${NC}"
echo -e "\n${BLUE}==============================================${NC}"
echo -e "${GREEN}ACTION REQUIRED: HARDWARE DECOUPLING${NC}"
echo -e "To ensure your backups survive container deletion,"
echo -e "copy ${YELLOW}init/proxmox-setup.sh${NC} to your PROXMOX HOST and run it:"
echo -e "${BLUE}# Run on root@proxmox:${NC}"
echo -e "${BLUE}bash proxmox-setup.sh${NC}"
echo -e "${BLUE}==============================================${NC}"
echo -e "Logs: journalctl -u atlas-backup.service -f"
echo -e "Config: /etc/atlas-backup/backup.conf"
echo -e "${BLUE}==============================================${NC}"
